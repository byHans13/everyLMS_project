...
public interface StudentDao {
	...
	@Select("SELECT DISTINCT CL_CLNAME,cl_idnum FROM vw_class WHERE aa_ID = #{mb_id}")
	...
	List<Clasc> searchClassQnaDetail(@Param("pageNum") int pageNum, @Param("search") String search,@Param("selvalue") String selvalue);// QNA 게시판 조건검색시 페이징
	List<Clasc> reviewlist(@Param("pageNum") int pageNum, @Param("search") String search); // REVIEW 검색시 페이징
	List<Clasc> searchClassReviewDetail(@Param("pageNum") int pageNum, @Param("search") String search,@Param("selvalue") String selvalue);//REVIEW 게시판 조건검색시 페이징
//총 게시물 카운트 
	...
	@Select("SELECT  * FROM vw_class WHERE CL_CLNAME = #{cl_clname} order by Co_NUM asc")
	List<Clasc> selectClassListAjax(String cl_clname);
