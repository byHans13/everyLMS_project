...
public class NomPaging {
	...
	public NomPaging(int maxNum, int pageNum, int listCount, int pageCount, String boardName) {
		this.maxNum = maxNum;
	...
	public NomPaging(int maxNum, Integer pageNum, int listCount, int pageCount, String boardName, String search) {
		this.maxNum = maxNum;
		this.pageNum = pageNum;
		this.listCount = listCount;
		this.pageCount = pageCount;
	...
	public String makeHtmlPaging() {
		...
		return makeHtml(currentGroup, totalPage, boardName , search);
	}


	private String makeHtml(int currentGroup, int totalPage, String boardName , String search) {
		...
		if (start != 1) {
			sb.append("<a href='"+boardName+"?search="+search+"&pageNum=" + (start -1) +"'>");
			sb.append("[이전]");
			sb.append("</a>");
		}
		...
		for (int i = start; i <= end; i++) {
			if (pageNum != i) { //현재 페이지가 아닌 경우 링크처리
				sb.append("<a href='"+boardName+"?search="+search+"&pageNum=" + i +"'>");
				sb.append(" [ ");
				sb.append(i);
				sb.append(" ] ");
				sb.append("</a>");
		...
		if (end != totalPage) {
			sb.append("<a href='"+boardName+"?search="+search+"&pageNum=" + (end + 1) + "'>");
			sb.append("[다음]");
			sb.append("</a>");
		}
		return sb.toString();
