...
<header>
   ...
   <section id="section" style="margin-left: 20px;">
    <div id='classMain' name='classMain' class='classMain'>
        ...
        <div id='classImg'>
          ...
          <ul class='ul'>
            <li id='classInfo'></li>
            <li id='className'></li>
            <li id='classLikeBtn'><input type='button' value='좋아요' onclick='classLike()'></li>
        </ul>
    ...
    function classInfoAjax(){ //강의소개 ajax
        ...
        var classReviews="";
        classReviews+="<div style='width:1036px; height:300px;'><br><br><div>수강후기 ";
        ...
        for(var i in infoReview){
        	...
        	classReviews+="<div style='width:1036px;' onclick=\"classReviewDetail('"+infoReview[i].cob_bonum+"','"+infoReview[i].gpa_gpa+"','"+infoReview[i].cob_kind+"')\">";
    ...
    function previewQuiz(){
    	...
    	window.open("selectPreviewQuiz?cl_idnum="+classPk+"&cl_lv="+el[0].cl_lv,'_blank','width=800, height=600, top=200, left=200'); 
    	
    }// previewQuiz() END
    
    function classLectureAjax(){ // 강의목록 ajax
    $("#classRight").html(""); //classRight 초기화 후 강의목록 테이블 찍어주기 위한 초기화 과정	
    var obj={"cl_idnum":el[0].cl_idnum ,"cl_lv":el[0].cl_lv};
    console.log(obj);
    	...
    	success: function(json){
    		...
    		console.log("atmk="+json[i].atd_atmk);
    				 if(json[i].atd_atmk !=null){
    					...
    					+json[i].co_idnum+"&co_lv="+json[i].co_lv+"&co_num="+json[i].co_num+"&atd_atmk="+json[i].atd_atmk+"' target='_blank'>"
    					+json[i].co_name+"</a></td><td>수강완료</td></tr>");    				
    				}else{
    					$('#lectureTable').append("<tr style='border-bottom:1px solid black;'><td>"
    					+json[i].co_num+"강</td><td><a href='selectClassLectureVideoPage?co_idnum="
    					+json[i].co_idnum+"&co_lv="+json[i].co_lv+"&co_num="+json[i].co_num+"' target='_blank'>"
