		...
		<input Type="hidden" value="${id}" id="hiddenid"> <select
			...
			<c:forEach var="cc" items="${CList}" varStatus="i">
				...
				<h2 id=viewlevel style="margin: auto; text-align: center;">나의 레벨=${viewlevel}</h2>
						<input type='hidden' value='${Clasc.cl_clname}'
							id="searchval">
		...
		<table  class="table table-striped">
			...
			<tbody id="tableShow">
				<c:forEach var="Clasc" items="${cList}">
					<tr>
						<td width="150px;">${Clasc.cl_cc}</td>
						<td width="150px;"><img
							src="<c:url value="/upload/${Clasc.pi_pisysname}"/>"
							width="150px;" /></td>
						<td width="150px;"><h2>${Clasc.cl_clname}|${Clasc.cl_id}|${Clasc.co_num} </h2> </td>
						<td width="150px;"><h2>${Clasc.cl_lv}</h2></td>
						<c:choose>
							<c:when test="${Clasc.gpa_gpa eq 0}">
								<td><h3>등록된 강의가 없습니다.</h3></td>
							</c:when>
							<c:when test="${Clasc.gpa_gpa ne 0}">
								<td width="150px;"><h2>${Clasc.gpa_gpa}</h2></td>
							</c:when>
						</c:choose>
						<td width="150px;"><a
